    ...
    for benchmark in benchmark_configs:   
        ...
        for i in range(repeats):
            ...
            for config in benchmark_configs[benchmark]:
                #outfile = result_folder_path+ "/" + benchmark + "_" + config.replace(" ","_") + "_result"
                cmd = benchmark_program + " " + benchmark + " " + config + " >> " + outfile + "_" + str(i+1)
                #print(outfile)
                print(cmd)
                #os.popen(cmd)
...
if (len(sys.argv) < 2):
    print("Usage: python3 run_microbenchmarks.py /path/to/config_file.yml")
    exit()
else:
    cfg_file = sys.argv[1]
