    ...
    if (lib_version == "pmdk"):
        ...
        for line in result_file: 
            ...
            if (line_category["info"]): #case of benchmark info line
                ...
                line_category["labels"] = False
                results[name].append([]) #init the list of the values      
            else: #number of lines to be read for values   
                if (line.startswith("total cycles") or line.startswith("thread")
                    or line.startswith("shortcut") or line.startswith("raw")):
                    continue
                elif (line.startswith("THREAD")):
                    labels = line.split(';')
                    if ("THREAD" not in results[name][list(line_category.keys()).index("labels")]):
                        results[name][list(line_category.keys()).index("labels")].extend(labels) # append the labels
                elif (line.startswith("total;")):
                    values = line.split(';')
                    results[name][list(line_category.keys()).index("values")][-1].extend(values)
                    if (int(line_category["values"]) == int(number_of_experiments)): #new benchmark in the next line
                        line_category["info"] = True
                        line_category["labels"] = False
                        line_category["values"] = 0
                else:
                    line_category["values"] = line_category["values"] + 1
                    values = line.split(';')
                    results[name][list(line_category.keys()).index("values")].append(values) # append the values
