# **Anchor Project : Code Repository**

## README.md forked and adapted from PMDK

Based on pmdk stable version 1.8
Development Branch

To compile Anchor, run in the root directory:
```
$ make clean && make clean ANCHOR_FUNCS=1
$ make ANCHOR_FUNCS=1 ENCR_OFF=0
```

For more information about the Anchor-related compile flags, please see [here](#anchor-build).

To reproduce the results shown in the evaluation of the paper, we provide (draft for now) [dedicated documentation](ARTIFACT.md).
...
### Anchor build
To build from source with ANCHOR functionalities set the ANCHOR_FUNCS variable:
```
        $ make clean
        $ make ANCHOR_FUNCS=1
        $ sudo make install ANCHOR_FUNCS=1 
        or
        $ make install ANCHOR_FUNCS=1 prefix=/absolute/path/for/build/directory/
```
To build from source with SCONE mmap function set the SCONE variable:
```
        $ make clean
        $ make SCONE=1
        $ sudo make install SCONE=1       
        or
        $ make install SCONE=1 prefix=/absolute/path/for/build/directory/
```
If you want both enabled, just combine the above steps:
```
        $ make clean
        $ make ANCHOR_FUNCS=1 SCONE=1
        $ sudo make install ANCHOR_FUNCS=1 SCONE=1
        or
        $ make install ANCHOR_FUNCS=1 SCONE=1 prefix=/absolute/path/for/build/directory/
```
Choice of enabling statistics measuring for read/write/alloc/free/tx/lookup:
```
        $ make clean
        $ make STATISTICS=1
        $ sudo make install STATISTICS=1
        or
        $ make install STATISTICS=1 prefix=/absolute/path/for/build/directory/
```
Choice of enabling write amplification measuring (statistics have to be enabled):
```
        $ make clean
        $ make STATISTICS=1 WRITE_AMPL=1
        $ sudo make install STATISTICS=1 WRITE_AMPL=1
        or
        $ make install STATISTICS=1 WRITE_AMPL=1 prefix=/absolute/path/for/build/directory/
```
Choice of enabling address sanitizer:
```
        $ make clean
        $ make SANITIZE=address
        $ sudo make install SANITIZE=address
        or
        $ make install SANITIZE=address prefix=/absolute/path/for/build/directory/
```
Choice of disabling encryption (on by default):
```
        $ make clean
        $ make ANCHOR_FUNCS=1 ENCR_OFF=1
        $ sudo make install ANCHOR_FUNCS=1 ENCR_OFF=1
        or
        $ make install ANCHOR_FUNCS=1 ENCR_OFF=1 prefix=/absolute/path/for/build/directory/
```

**Important Notes for Networking:**
1. Native installation instructions: https://github.com/saendigPhilip/network-stack/tree/master
2. Scone installation instructions: https://github.com/dgiantsidi/anchor-eRPC/tree/main/eRPC/build
3. To reproduce the results you have to set: MAX_PENDING_REQUESTS = 16 in client_server_two_side/src/client_server_common.h
