...
do
    for i in $REPEATS
    do
        LD_PRELOAD=$PRELOAD_LIBS Hugepagesize=2097152 LD_LIBRARY_PATH=/usr/lib/x86_64-linux-gnu/:/usr/lib/gcc/x86_64-linux-gnu/7/ SCONE_VERSION=1 SCONE_LOG=0 SCONE_QUEUES=8 SCONE_NO_FS_SHIELD=1 SCONE_NO_MMAP_ACCESS=1 SCONE_HEAP=8G SCONE_LD_DEBUG=1 SCONE_MODE=hw /opt/scone/lib/ld-scone-x86_64.so.1 ./pmembench anchor_pmembench_map_"$ds".cfg | tee ../../results/anchor_encr_scone/anchor_pmembench_map_"$ds"_result_"$i"
    done
done
...
do
    for i in $REPEATS
    do
    LD_PRELOAD=$PRELOAD_LIBS Hugepagesize=2097152 LD_LIBRARY_PATH=/usr/lib/x86_64-linux-gnu/:/usr/lib/gcc/x86_64-linux-gnu/7/ SCONE_VERSION=1 SCONE_LOG=0 SCONE_QUEUES=8 SCONE_NO_FS_SHIELD=1 SCONE_NO_MMAP_ACCESS=1 SCONE_HEAP=8G SCONE_LD_DEBUG=1 SCONE_MODE=hw /opt/scone/lib/ld-scone-x86_64.so.1 ./pmembench anchor_pmembench_map_"$ds".cfg | tee ../../results/anchor_no_encr_scone/anchor_pmembench_map_"$ds"_result_"$i"
    done
done
